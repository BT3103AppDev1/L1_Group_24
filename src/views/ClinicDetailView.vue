          ...
          <li v-for="svcId in clinic.services" :key="svcId" class="service-item"
            ...
            <div class="service-info">
              ...
              <span class="service-time">
                Est. wait: ~{{ clinic.waitByService?.[svcId] ?? clinic.averageWaitTime }} min
...
const formattedHours = computed(() => {
  // If manual override is active, show that instead of a schedule
  if (clinic.value?.isOpen) {
    return 'Currently Open (Manual Override)'
  }
...
const isOpen = computed(() => {
  if (clinic.value?.isOpen !== undefined) {
    return clinic.value.isOpen
  }
...
async function handleJoinQueue() {
  ...
  // (future: show a modal prompt instead)
