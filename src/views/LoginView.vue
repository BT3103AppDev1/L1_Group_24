  ...
  <AuthLayout>
    ...
    <template v-if="role === 'patient'">
      ...
      <form @submit.prevent="submitPatientLogin" class="auth-form" novalidate>
        <AppInput v-model="pl.email" label="Email Address" type="email" placeholder="you@email.com" :error="pl.errors.email" required />
        ...
        <AlertBanner v-if="pl.serverError" type="error" :message="pl.serverError" dismissible @dismiss="pl.serverError=''" />
    ...
    <template v-else>
      ...
      <form @submit.prevent="submitClinicLogin" class="auth-form" novalidate>
        <AppInput v-model="cl.email" label="Email Address" type="email" placeholder="clinic@email.com" :error="cl.errors.email" required />
        <AppInputPassword v-model="cl.password" label="Password" :error="cl.errors.password" required />
        <AlertBanner v-if="cl.serverError" type="error" :message="cl.serverError" dismissible @dismiss="cl.serverError=''" />
        <AppButton type="submit" variant="primary" block :loading="cl.loading">Log In</AppButton>
      </form>
      <p class="auth-footer">New clinic? <RouterLink to="/clinic/register">Register here</RouterLink></p>
    </template>
  </AuthLayout>
</template>
...
.form-heading { font-size: 1.4rem; font-weight: 800; color: #1f2937; margin-bottom: .25rem; }
.form-sub     { color: #6b7280; font-size: .875rem; margin-bottom: 1.5rem; }
.auth-form    { display: flex; flex-direction: column; gap: 1rem; }
.forgot-row   { text-align: right; }
.forgot-link  { font-size: .8rem; color: #3b82f6; font-weight: 500; }
.auth-footer  { text-align: center; margin-top: 1rem; font-size: .875rem; color: #6b7280; }
